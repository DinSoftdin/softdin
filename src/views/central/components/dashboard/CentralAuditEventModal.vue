...
import type { CentralAuditEventDetailResponse } from '@/types/central-audit'
import {
  auditCategoryLabel,
  auditDisplayValue,
  auditFieldPathLabel,
  auditResultLabel,
} from '@/utils/central-audit-labels'
...
const detail = ref<CentralAuditEventDetailResponse | null>(null)
...
const summaryText = computed(() => {
  ...
  const action = event.category_label ?? auditCategoryLabel(event.event_category)
...
function fieldLabel(change: { field_label?: string; field_path: string }): string {
  return change.field_label ?? auditFieldPathLabel(change.field_path)
}

function valueLabel(change: {
  ...
  if (key === 'old') {
    return change.old_value_label ?? auditDisplayValue(change.old_value)
  }

  return change.new_value_label ?? auditDisplayValue(change.new_value)
}
...
async function loadDetail(): Promise<void> {
  ...
  try {
    detail.value = await centralAuditService.fetchEvent(props.eventId)
  } catch (err) {
    ...
    <div v-if="open" class="modal-backdrop" @click.self="close">
      <div class="modal-panel" role="dialog" aria-modal="true" aria-labelledby="audit-modal-title">
        <header class="modal-header">
          <div>
            <p class="modal-kicker">Auditoría central</p>
        ...
        <div class="modal-body">
          ...
          <template v-else-if="detail">
            ...
            <dl class="meta-grid">
              <div>
                <dt>Acción</dt>
                <dd>{{ detail.event.category_label ?? auditCategoryLabel(detail.event.event_category) }}</dd>
              </div>
              <div>
                <dt>Resultado</dt>
                <dd>{{ auditResultLabel(detail.event.result) }}</dd>
              </div>
              <div>
                <dt>Fecha</dt>
